...
inline constexpr const char* kMempoolName = "ot_almp";
...
optitrade::wire::EthernetEnvelope response_envelope(
    const optitrade::wire::EthernetEnvelope& request) noexcept {
...
int main(int argc, char** argv) {
    if (rte_eal_init(argc, argv) < 0) {
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 9C failure: EAL initialization failed\n");
        return EXIT_FAILURE;
    }
    ...
    if (tsc_hz == 0) {
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 9C failure: invalid TSC frequency\n");
        rte_eal_cleanup();
        return EXIT_FAILURE;
    }
    ...
    if (available_ports == 0) {
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 9C failure: no AF_PACKET DPDK port available\n");
        rte_eal_cleanup();
        return EXIT_FAILURE;
    }
    ...
    if (!found_port) {
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 9C failure: port enumeration failed\n");
        rte_eal_cleanup();
        return EXIT_FAILURE;
    }
    ...
    if (mempool == nullptr) {
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 9C failure: mempool creation failed: %s\n",
    ...
    std::printf("============================================================\n");
    std::printf("OptiTrade Engine Level 3 Phase 9C - AF_PACKET Latency Engine\n");
    std::printf("============================================================\n");
    ...
    if (success) {
        ...
        std::printf("\n============================================================\n");
        std::printf("OptiTrade Engine Phase 9C Benchmark Result\n");
        std::printf("============================================================\n");
        ...
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 9C failure: benchmark did not complete correctly\n");
    }
