...
#include "optitrade/dpdk/dpdk_config.hpp"
#include "optitrade/wire/ethernet_frame.hpp"

namespace {

using optitrade::dpdk::kBurstSize;
using optitrade::dpdk::kDataRoomSize;
using optitrade::dpdk::kEnginePortName;
using optitrade::dpdk::kGeneratorPortName;
using optitrade::dpdk::kMarketDataRingName;
using optitrade::dpdk::kMempoolCacheSize;
using optitrade::dpdk::kMempoolName;
using optitrade::dpdk::kMempoolSize;
using optitrade::dpdk::kOutboundOrderRingName;
using optitrade::dpdk::kReceivePollAttempts;
using optitrade::dpdk::kRingSize;
using optitrade::dpdk::kRxDescriptors;
using optitrade::dpdk::kRxQueues;
using optitrade::dpdk::kTxDescriptors;
using optitrade::dpdk::kTxQueues;

using optitrade::wire::EthernetEnvelope;
using optitrade::wire::EthernetFrame;
using optitrade::wire::FrameDecodeError;
using optitrade::wire::MarketDataMessage;
using optitrade::wire::OrderType;
using optitrade::wire::OutboundOrderMessage;
using optitrade::wire::Side;
using optitrade::wire::TimeInForce;
using optitrade::wire::UpdateAction;
using optitrade::wire::decode_market_data_frame;
using optitrade::wire::decode_order_frame;
using optitrade::wire::encode_market_data_frame;
using optitrade::wire::encode_order_frame;
using optitrade::wire::kEthernetFrameBytesWithoutFcs;
...
void print_failure(const char* operation, const int error_code) noexcept {
    std::fprintf(stderr,
                 "OptiTrade Engine Phase 4 failure: %s failed, code=%d, reason=%s\n",
                 operation,
                 error_code,
...
bool create_virtual_ports(rte_mempool* const mempool,
    ...
    if (market_data_ring == nullptr) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: cannot create market-data ring: %s\n",
    ...
    if (outbound_order_ring == nullptr) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: cannot create order ring: %s\n",
                     rte_strerror(rte_errno));
        return false;
    }
    ...
    if (generator_port < 0) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: cannot create generator port\n");
        return false;
    }
    ...
    if (engine_port < 0) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: cannot create engine port\n");
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    if (eal_arguments < 0) {
        std::fprintf(stderr, "OptiTrade Engine Phase 4 failure: EAL initialization failed\n");
        return EXIT_FAILURE;
    }

    std::printf("============================================================\n");
    std::printf("OptiTrade Engine Level 3 Phase 4 - Virtual DPDK RX/TX Path\n");
    std::printf("============================================================\n");
    ...
    if (mempool == nullptr) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: cannot create mbuf pool: %s\n",
                     rte_strerror(rte_errno));
        rte_eal_cleanup();
        return EXIT_FAILURE;
    }
    ...
    if (!encode_market_data_frame(
        ...
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: market frame encoding failed\n");
    ...
    if (market_packet == nullptr) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: market mbuf allocation failed\n");
        close_port(ports.engine);
        close_port(ports.generator);
        rte_eal_cleanup();
        return EXIT_FAILURE;
    ...
    if (!transmit_single_packet(ports.generator, market_packet)) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: market packet TX failed\n");
        close_port(ports.engine);
        close_port(ports.generator);
        rte_eal_cleanup();
        return EXIT_FAILURE;
    ...
    if (engine_received_packet == nullptr) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: engine RX timed out\n");
        close_port(ports.engine);
        close_port(ports.generator);
        rte_eal_cleanup();
        return EXIT_FAILURE;
    ...
    if (rte_pktmbuf_data_len(engine_received_packet) <
        ...
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: received short market frame\n");
    ...
    if (market_decode_result != FrameDecodeError::none) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: received market frame invalid, code=%u\n",
    ...
    if (!encode_order_frame(
            ...
            encoded_order_frame)) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: order frame encoding failed\n");
        close_port(ports.engine);
        close_port(ports.generator);
        rte_eal_cleanup();
        return EXIT_FAILURE;
    ...
    if (order_packet == nullptr) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: order mbuf allocation failed\n");
        close_port(ports.engine);
        close_port(ports.generator);
        rte_eal_cleanup();
        return EXIT_FAILURE;
    ...
    if (!transmit_single_packet(ports.engine, order_packet)) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: order packet TX failed\n");
        close_port(ports.engine);
        close_port(ports.generator);
        rte_eal_cleanup();
        return EXIT_FAILURE;
    ...
    if (generator_received_packet == nullptr) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: generator RX timed out\n");
        close_port(ports.engine);
        close_port(ports.generator);
        rte_eal_cleanup();
        return EXIT_FAILURE;
    ...
    if (order_decode_result != FrameDecodeError::none) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: received order frame invalid, code=%u\n",
    ...
    if (!packet_data_correct) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: packet field validation failed\n");
        close_port(ports.engine);
        close_port(ports.generator);
        rte_eal_cleanup();
        return EXIT_FAILURE;
    ...
    if (cleanup_result != 0) {
        std::fprintf(stderr,
                     "OptiTrade Engine Phase 4 failure: EAL cleanup failed\n");
        return EXIT_FAILURE;
    }
