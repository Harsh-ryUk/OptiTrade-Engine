...
bool run_order_iteration(
    optitrade::dpdk::LatencyVirtualLink& link,
    optitrade::TradingEngine<64>& engine,
    const optitrade::wire::EthernetEnvelope& market_envelope,
    const optitrade::wire::EthernetEnvelope& order_envelope,
    ...
    if (result.status != optitrade::EngineStatus::order_emitted ||
        result.signal != optitrade::Signal::buy) {
        ++counters.failures;
        return false;
    }
    ...
    optitrade::wire::EthernetFrame drained_order_frame{};
...
int main(int argc, char** argv) {
    if (rte_eal_init(argc, argv) < 0) {
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 6 failure: EAL initialization failed\n");
        return EXIT_FAILURE;
    }
    ...
    if (tsc_hz == 0) {
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 6 failure: invalid TSC frequency\n");
        rte_eal_cleanup();
        return EXIT_FAILURE;
    }
    ...
    optitrade::dpdk::DpdkLatencyRecorder latency_recorder(
        kMeasuredEvents,
        tsc_hz);
...

    {
        ...
        if (!link.initialize()) {
            std::fprintf(
                stderr,
                "OptiTrade Engine Phase 6 failure: virtual DPDK link initialization failed: %s\n",
            ...
            if (success &&
                ...
                std::fprintf(
                    stderr,
                    "OptiTrade Engine Phase 6 failure: book seeding failed\n");
                success = false;
            }
            ...
            if (success) {
                ...
                std::printf("============================================================\n");
                std::printf("OptiTrade Engine Level 3 Phase 6 - Virtual DPDK Latency Benchmark\n");
                std::printf("============================================================\n");
