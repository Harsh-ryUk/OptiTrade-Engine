...
#include "optitrade/dpdk/dpdk_latency_recorder.hpp"
#include "optitrade/dpdk/engine_wire_adapter.hpp"
#include "optitrade/dpdk/latency_virtual_link.hpp"
...
optitrade::wire::EthernetEnvelope make_market_envelope() noexcept {
...
optitrade::wire::EthernetEnvelope make_order_envelope() noexcept {
    optitrade::wire::EthernetEnvelope envelope{};
...
optitrade::wire::MarketDataMessage make_regime_message(
    const Regime regime,
    const std::uint32_t sequence_number,
...
bool seed_balanced_book(
    optitrade::dpdk::LatencyVirtualLink& link,
    ...
    for (std::uint16_t level = 0; level < 5; ++level) {
        ...
        if (decode_result != optitrade::wire::FrameDecodeError::none) {
            return false;
        }
...
bool validate_order(
    const optitrade::wire::EthernetFrame& frame,
    ...
    if (regime == Regime::buy) {
        return order.side == optitrade::wire::Side::buy &&
               order.price_ticks == 100100;
    }

    if (regime == Regime::sell) {
        return order.side == optitrade::wire::Side::sell &&
...
bool run_regime_iteration(
    ...
    optitrade::wire::EthernetFrame incoming_frame{};
    ...
    const optitrade::Signal expected_signal =
        regime == Regime::buy
            ? optitrade::Signal::buy
            : optitrade::Signal::sell;
    ...
    optitrade::wire::EthernetFrame outbound_frame{};
...
bool benchmark_regime(
    ...
    optitrade::dpdk::LatencySummary& output_summary,
    ...
    optitrade::dpdk::DpdkLatencyRecorder recorder(
...
int main(int argc, char** argv) {
    if (rte_eal_init(argc, argv) < 0) {
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 8 failure: EAL initialization failed\n");
        return EXIT_FAILURE;
    }
    ...
    if (tsc_hz == 0) {
        std::fprintf(
            stderr,
            "OptiTrade Engine Phase 8 failure: invalid TSC frequency\n");
        rte_eal_cleanup();
        return EXIT_FAILURE;
    }

    optitrade::dpdk::DpdkLatencyRecorder timer_overhead(
    ...
    optitrade::dpdk::LatencySummary no_signal_summary{};
    optitrade::dpdk::LatencySummary buy_summary{};
    optitrade::dpdk::LatencySummary sell_summary{};

    RegimeCounters no_signal_counters{};
    RegimeCounters buy_counters{};
    RegimeCounters sell_counters{};

    optitrade::dpdk::LatencyLinkStatistics virtual_stats{};

    {
        ...
        if (!link.initialize()) {
            std::fprintf(
                stderr,
                "OptiTrade Engine Phase 8 failure: virtual DPDK link initialization failed: %s\n",
        ...
        if (success &&
            ...
            std::fprintf(
                stderr,
                "OptiTrade Engine Phase 8 failure: NO_SIGNAL benchmark failed\n");
            success = false;
        }
        ...
        if (success &&
            ...
            std::fprintf(
                stderr,
                "OptiTrade Engine Phase 8 failure: BUY benchmark failed\n");
            success = false;
        }
        ...
        if (success &&
            ...
            std::fprintf(
                stderr,
                "OptiTrade Engine Phase 8 failure: SELL benchmark failed\n");
            success = false;
        }
    ...
    if (success) {
        ...
        std::printf("============================================================\n");
        std::printf("OptiTrade Engine Level 3 Phase 8 - Regime Latency Benchmark\n");
        std::printf("============================================================\n");
